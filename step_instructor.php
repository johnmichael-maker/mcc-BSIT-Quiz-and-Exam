...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!str_ends_with($email, $domain)) {
        ...
        $stmt = $conn->prepare("SELECT email FROM signupinstructors WHERE email = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO signupinstructors (email, reset_token_hash, reset_token_hash_expires_at) VALUES (?, ?, ?)");
