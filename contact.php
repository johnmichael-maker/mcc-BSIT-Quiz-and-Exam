...
$username = "u510162695_windale"; // your MySQL username
$password = "1Windale_pass"; // your MySQL password
$dbname = "u510162695_windale"; // your database name
...
if (isset($_POST['edit_record'])) {
    $selected_table = $_POST['selected_table'];
    $id = $_POST['id'];  // Changed admin_id to id
    ...
    if ($row) {
        ...
        echo "<h3>Edit Data for ID $id</h3>";  // Changed admin_id to id
        echo "<form method='POST' action=''>";
        echo "<input type='hidden' name='selected_table' value='$selected_table'>";
        echo "<input type='hidden' name='id' value='$id'>";  // Changed admin_id to id
        ...
        echo "<div class='alert error'>No record found for ID $id.</div>";  // Changed admin_id to id
    }
}
...
if (isset($_POST['save_edit'])) {
    ...
    foreach ($_POST as $key => $value) {
        if ($key != 'selected_table' && $key != 'id' && $key != 'save_edit') {  // Changed admin_id to id
...
if (isset($_POST['view_columns'])) {
    $selected_table = $_POST['selected_table'];
    echo "<h3>Enter ID to Edit Data</h3>";  // Changed Admin ID to ID
    echo "<form method='POST' action=''>";
    echo "<input type='hidden' name='selected_table' value='$selected_table'>";
    echo "<label for='id'>ID</label>";  // Changed label to ID
    echo "<input type='text' name='id' id='id' required><br><br>";  // Changed input to id
